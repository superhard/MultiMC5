...
public:
	...
	static ResourcePtr create(const QString &resource);

	/// This can e.g. be used to set a local icon as the placeholder while a slow (remote) icon is fetched
	ResourcePtr placeholder(ResourcePtr other);

	/// Use these functions to specify what should happen when e.g. the resource changes
	ResourcePtr applyTo(ResourceObserver *observer);
	ResourcePtr applyTo(QObject *target, const char *property = nullptr);
	template<typename Func>
	ResourcePtr then(Func &&func)
	{
...
private:
	...
	ResourcePtr m_placeholder = nullptr;
